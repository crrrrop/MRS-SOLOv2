...
class FPN(paddle.nn.Layer):
    ...
    def forward(self, body_feats):
        ...
        '''
        # print("|________________________FPN_____________________________")
        spatial_scale = copy.deepcopy(self.spatial_scale)
        num_backbone_stages = self.num_backbone_stages   # 进入FPN的张量个数
        # for i in range(0,len(body_feats)):
            # print(f"|||body_feats:{body_feats[i].shape}")
        ...
        # print(f"||num_backbone_stages:{num_backbone_stages}")
        # print(f"||body_feats` len:{len(body_feats)}")
