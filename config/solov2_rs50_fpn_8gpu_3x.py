...
class SOLOv2_rs50_fpn_8gpu_3x_Config(object):
    def __init__(self):
        # 自定义数据集
        ...
        # ========= 一些设置 =========
        self.train_cfg = dict(
            # 关于epoch的计算：假设训练集有n个数据，那么batch_num(batch的个数)=n/batch_size；epoch=max_iters/batch_num
            batch_size=2,
            num_threads=5,   # 读数据的线程数
            max_batch=2,     # 最大读多少个批
            model_path= None,  # "./weights/best_model.pdparams" 
            save_iter=1750,   # 每隔几步保存一次模型 10个epoch
            eval_iter=5250,   # 每隔几步计算一次eval集的mAP 30个epoch
            max_iters=42000,   # 训练多少步 350张训练数据， batch_num(batch的个数)=350/2=175;epoch=max_iters/batch_num=26250/175=150
        ...
        self.learningRate = dict(
            ...
            PiecewiseDecay=dict(  # 分段衰减配置
                gamma=0.0001,  # 学习率衰减系数，每次衰减时将学习率乘以这个系数
                milestones=[180000, 240000],  # 学习率衰减的步数，达到这些步数时进行衰减
            ),
            LinearWarmup=dict(  # 线性预热配置
                start_factor=0.01,  # 预热的起始因子，即预热开始时的学习率为 base_lr * start_factor
                steps=1000,  # 预热的步数，在这段步数内线性增加学习率
            ),
        )
        ...
        # 测试。用于demo.py
        self.test_cfg = dict(
            model_path='weights_St+MSCA34/best_model.pdparams',   # dygraph_solov2_r50_fpn_8gpu_3x.pdparams
            # model_path='./weights/1000.pdparams',
            target_size=900,
        ...
        self.backbone_type = 'ResneSt50'  # 使用Resnet50Vb作为骨干网络
        self.backbone = dict(  # 骨干网络的配置参数
            ...
            dcn_v2_stages=[],  # 使用可变形卷积的阶段 3,4,5
